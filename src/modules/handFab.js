...
export function renderHandFab() {
    ...
    if (appState.loadingTab === 'hand' || (appState.isLoading && appState.parts.hand.length === 0)) {
        const row = document.createElement("tr");
        row.innerHTML = `<td colspan="7" class="text-center py-8 text-gray-500"><div class="flex items-center justify-center"><i class="fa-solid fa-spinner fa-spin text-purple-400 mr-2"></i> Loading hand fabrication parts...</div></td>`;
